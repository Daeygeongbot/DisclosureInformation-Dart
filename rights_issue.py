...
# =========================================================
# 1. GitHub Secrets / 환경변수
# =========================================================
dart_key = os.environ["DART_API_KEY"]
service_account_str = os.environ["GOOGLE_CREDENTIALS_JSON"]
sheet_id = os.environ["GOOGLE_SHEET_ID"]

RIGHTS_SHEET_NAME = os.getenv("RIGHTS_SHEET_NAME", "D_유상증자")
...
DETAIL_URLS = {
    "유": "https://opendart.fss.or.kr/api/piicDecsn.json",
    "무": "https://opendart.fss.or.kr/api/fricDecsn.json",
    "유무": "https://opendart.fss.or.kr/api/pifricDecsn.json",
}
...
def detect_report_kind(report_nm):
    t = str(report_nm or "").replace(" ", "")
    if "유무상증자결정" in t:
        return "유무"
    if "무상증자결정" in t:
        return "무"
...
# DART 필드 접근 헬퍼
# =========================================================
def get_section_field(row, report_kind, section, field_name):
    """
    report_kind: 유 / 무 / 유무
    section: rights / bonus
    """
    if report_kind == "유":
        if section != "rights":
            return None
    ...
    if report_kind == "유무":
        prefix = "piic_" if section == "rights" else "fric_"
        return row.get(prefix + field_name)

    return None


def get_share_summary(row, report_kind, section):
    ostk = to_int(get_section_field(row, report_kind, section, "nstk_ostk_cnt"))
    estk = to_int(get_section_field(row, report_kind, section, "nstk_estk_cnt"))
    ...
    old_ostk = to_int(get_section_field(row, report_kind, section, "bfic_tisstk_ostk"))
    old_estk = to_int(get_section_field(row, report_kind, section, "bfic_tisstk_estk"))
    old_shares = old_ostk + old_estk

    return new_shares, product, old_shares


def get_rights_total_amount_and_purpose(row, report_kind):
    fclt = to_int(get_section_field(row, report_kind, "rights", "fdpp_fclt"))
    bsninh = to_int(get_section_field(row, report_kind, "rights", "fdpp_bsninh"))
    op = to_int(get_section_field(row, report_kind, "rights", "fdpp_op"))
    dtrp = to_int(get_section_field(row, report_kind, "rights", "fdpp_dtrp"))
    ocsa = to_int(get_section_field(row, report_kind, "rights", "fdpp_ocsa"))
    etc = to_int(get_section_field(row, report_kind, "rights", "fdpp_etc"))
    ...
    return total_amt, purpose_str


# =========================================================
# 행 데이터 생성
# =========================================================
def make_rights_row(row, xml_data, cls_map, report_kind="유", output_kind="유"):
    rcept_no = str(row.get("rcept_no", ""))
    corp_name = row.get("corp_name", "")
    report_nm = row.get("report_nm", "")

    market = cls_map.get(row.get("corp_cls", ""), "기타")
    method = str(first_nonempty(get_section_field(row, report_kind, "rights", "ic_mthn"))).strip() or "-"

    new_shares, product, old_shares = get_share_summary(row, report_kind, "rights")
    total_amt, purpose_str = get_rights_total_amount_and_purpose(row, report_kind)
    ...
    return [
        ...
        report_nm,                      # 3 보고서명
        market,                         # 4 상장시장
        xml_data["board_date"],         # 5 최초 이사회결의일
        method,                         # 6 증자방식
        product,                        # 7 발행주식종류
        new_shares_str,                 # 8 신규발행주식수
        issue_price_str,                # 9 확정발행가(원)
        base_price_str,                 # 10 기준주가
        total_amt_uk,                   # 11 확정발행금액(억원)
        discount_str,                   # 12 할인(할증률)
        old_shares_str,                 # 13 증자전 주식수
        ratio,                          # 14 증자비율
        xml_data["pay_date"],           # 15 납입일
        xml_data["div_date"],           # 16 배당기산일
        xml_data["list_date"],          # 17 상장예정일
        xml_data["board_date"],         # 18 이사회결의일
        purpose_str,                    # 19 자금용도
        xml_data["investor"],           # 20 투자자
        link,                           # 21 링크
        rcept_no,                       # 22 접수번호
    ]


def make_bonus_row(row, xml_data, cls_map, report_kind="무", output_kind="무"):
    rcept_no = str(row.get("rcept_no", ""))
    corp_name = row.get("corp_name", "")
    report_nm = row.get("report_nm", "")

    market = cls_map.get(row.get("corp_cls", ""), "기타")
    new_shares, product, old_shares = get_share_summary(row, report_kind, "bonus")

    board_date = first_nonempty(
        fix_date(get_section_field(row, report_kind, "bonus", "bddd")),
        xml_data["board_date"],
    )
    div_date = first_nonempty(
        fix_date(get_section_field(row, report_kind, "bonus", "nstk_dividrk")),
        xml_data["div_date"],
    )
    list_date = first_nonempty(
        fix_date(get_section_field(row, report_kind, "bonus", "nstk_lstprd")),
        xml_data["list_date"],
    )
...
# DART 상세 데이터 수집
# =========================================================
def fetch_rights_bonus_detail_df(df_filtered, start_date, end_date):
    detail_frames = []

    detail_start_date = (
        datetime.strptime(start_date, "%Y%m%d") - timedelta(days=DETAIL_LOOKBACK_DAYS)
    ).strftime("%Y%m%d")

    for report_kind in ["유", "무", "유무"]:
        df_kind = df_filtered[df_filtered["report_kind"] == report_kind].copy()
        if df_kind.empty:
            continue

        corp_codes = df_kind["corp_code"].astype(str).unique()
        target_rcept_nos = set(df_kind["rcept_no"].astype(str).tolist())
        ...
        for code in corp_codes:
            ...
            df_detail = fetch_dart_json(DETAIL_URLS[report_kind], detail_params)
            if not df_detail.empty:
                kind_detail_dfs.append(df_detail)
            time.sleep(0.1)

        if not kind_detail_dfs:
            continue

        df_kind_detail = pd.concat(kind_detail_dfs, ignore_index=True)
        df_kind_detail["rcept_no"] = df_kind_detail["rcept_no"].astype(str)
        df_kind_detail = df_kind_detail[df_kind_detail["rcept_no"].isin(target_rcept_nos)].copy()
        df_kind_detail = df_kind_detail.drop_duplicates(subset=["rcept_no"], keep="last")
        detail_frames.append(df_kind_detail)
...
def get_and_update_rights_bonus():
    ...
    # 2) report_nm으로 유상/무상/유무상만 필터
    df_filtered = all_filings.copy()
    df_filtered["report_kind"] = df_filtered["report_nm"].apply(detect_report_kind)
    df_filtered = df_filtered[df_filtered["report_kind"].notna()].copy()

    if df_filtered.empty:
        print("ℹ️ 유상/무상/유무상 공시가 없습니다.")
        return

    df_filtered["rcept_no"] = df_filtered["rcept_no"].astype(str)
    df_filtered = df_filtered.drop_duplicates(subset=["rcept_no"], keep="last")

    df_report_map = df_filtered[
        ["rcept_no", "report_nm", "report_kind"]
    ].drop_duplicates(subset=["rcept_no"], keep="last")

    # 3) 상세 API는 유형별 endpoint 분기
    df_detail = fetch_rights_bonus_detail_df(df_filtered, start_date, end_date)

    if df_detail.empty:
        print("ℹ️ 상세 데이터를 불러올 수 없습니다.")
        return

    df_detail["rcept_no"] = df_detail["rcept_no"].astype(str)
    df_merged = df_detail.merge(df_report_map, on="rcept_no", how="left")
    df_merged = df_merged.drop_duplicates(subset=["rcept_no"], keep="last")

    if df_merged.empty:
        print("ℹ️ 최종 병합된 유상/무상/유무상 상세 데이터가 없습니다.")
        return
